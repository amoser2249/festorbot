...
Client.once('ready', () => {
  ...
  setInterval(() => {
    Config.SERVERS.forEach((Server) => {
      console.debug(
        `Polling server ${Server.NAME} (${Server.ADDRESS}:${Server.PORT})...`
      );
      ...
      Gamedig.query({
        ...
        .catch((error) => {
          console.error(
            `Unable to query game server! (${Server.ADDRESS}:${Server.PORT})`
          );
          console.error(error);
  ...
  console.log(
    `Polling game servers every ${PollingIntervalSeconds} seconds...`
  );
});
